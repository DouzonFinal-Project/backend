...
# ✅ [READ] 특정 학생 생활기록부
@router.get("/student/{student_id}", response_model=List[SchoolReportSchema])
...
@router.get("/class/{class_id}", response_model=List[SchoolReportSchema])
def get_class_school_reports(class_id: int, db: Session = Depends(get_db)):
    reports = (
        db.query(SchoolReportModel)
        .join(StudentModel, StudentModel.id == SchoolReportModel.student_id)  # ✅ 학생 테이블과 조인
